class Brandnew::CarsController < ApplicationController
  ...
  price_from=params[:PriceFrom] || session[:PriceFrom]
  price_to=params[:PriceTo] || session[:PriceTo]
  
  if params[:car_name]!=session[:car_name] or params[:PriceFrom] != session[:PriceFrom] or params[:PriceTo] != session[:PriceTo]

  session[:car_name]=car_id
  session[:PriceFrom]=price_from
  session[:PriceTo]=price_to
